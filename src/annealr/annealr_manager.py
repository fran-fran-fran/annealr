...
#
# File: annealr_manager.py
# Description: AnnealrManager - main plugin class. Registers GCode commands,
...
from .state_machine import AnnealrStateMachine
...
class AnnealrManager:
    ...
    def __init__(self, config):
        ...
        self.log = logging.getLogger('annealr')
    ...
    def register_profile(self, profile):
        """Register a profile (called by annealr_profile load_config_prefix)."""
        self._profiles[profile.name] = profile
        self.log.info(
            "Loaded annealr profile: %s (%d segments)",
    ...
    def _register_commands(self):
        g = self.gcode
        g.register_command(
            'ANNEALR_PROFILE_BEGIN', self.cmd_profile_begin,
            desc="Begin defining a new annealr profile")
        g.register_command(
            'ANNEALR_PROFILE_RAMP', self.cmd_profile_ramp,
            desc="Add a ramp segment to the profile being defined")
        g.register_command(
            'ANNEALR_PROFILE_SOAK', self.cmd_profile_soak,
            desc="Add a soak segment to the profile being defined")
        g.register_command(
            'ANNEALR_PROFILE_END', self.cmd_profile_end,
            desc="Finish and commit the profile being defined")
        g.register_command(
            'ANNEALR_PROFILE_ABORT', self.cmd_profile_abort,
            desc="Discard the profile being defined")
        g.register_command(
            'ANNEALR_PROFILES', self.cmd_list_profiles,
            desc="List all available annealr profiles")
        g.register_command(
            'ANNEALR_PROFILE_SHOW', self.cmd_show_profile,
            desc="Show details of a specific profile")
        g.register_command(
            'ANNEALR_PROFILE_DELETE', self.cmd_delete_profile,
            desc="Delete a saved profile")
        g.register_command(
            'ANNEALR_START', self.cmd_start,
            desc="Start an annealing run")
        g.register_command(
            'ANNEALR_PAUSE', self.cmd_pause,
            desc="Pause the active annealing run")
        g.register_command(
            'ANNEALR_RESUME', self.cmd_resume,
            desc="Resume a paused annealing run")
        g.register_command(
            'ANNEALR_CANCEL', self.cmd_cancel,
            desc="Cancel the active annealing run")
        g.register_command(
            'ANNEALR_STATUS', self.cmd_status,
    ...
    def cmd_profile_begin(self, gcmd):
        if self._pending is not None:
            raise gcmd.error(
                ...
                "Run ANNEALR_PROFILE_END to commit or "
                "ANNEALR_PROFILE_ABORT to discard."
                % self._pending.name)
        ...
        self._respond("Defining profile '%s'. Add segments with "
                      "ANNEALR_PROFILE_RAMP and ANNEALR_PROFILE_SOAK."
                      % name)
    ...
    def cmd_profile_ramp(self, gcmd):
        if self._pending is None:
            raise gcmd.error(
                ...
                "Run ANNEALR_PROFILE_BEGIN first.")
    ...
    def cmd_delete_profile(self, gcmd):
        ...
        self._respond("Profile '%s' deleted from runtime store. "
                      "Remove [annealr_profile %s] from printer.cfg "
    ...
    def cmd_pause(self, gcmd):
        ...
        self._respond("Annealing paused. Heater holding. "
                      "Run ANNEALR_RESUME to continue.")
    ...
    def _tick(self, eventtime):
        ...
        try:
            ...
            self.log.exception("Error in annealr tick")
    ...
    def _stage_profile_for_save(self, p):
        ...
        try:
            ...
            section_name = "annealr_profile %s" % p.name
